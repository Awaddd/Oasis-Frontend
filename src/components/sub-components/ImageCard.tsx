...
const Container: FC<{ link?: string; children: React.ReactNode }> = ({ link, children }) => {
  ...
  return (<Link href={link} passHref>{children}</Link>);
}
  ...
  return (
    <Container link={link}>
      <div className={classNames("relative h-52 sm:h-60 md:h-80 lg:h-96 3xl:h-[28rem]", classes)}>
        {imageProps && (<Image layout="fill" {...imageProps} placeholder="blur" priority alt={title} className="absolute top-0 z-10 object-cover text-center text-gray-200 bg-gray-900 rounded-lg heroImage" />)}
