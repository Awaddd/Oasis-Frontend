...
import { ARTICLE, ARTICLES, ARTICLES_BY_CATEGORY, FEATURED_ARTICLE, FIRST_THREE_ARTICLES } from "./queries"
...
export async function getArticles(getFirstThree?: true) {
  const data = (getFirstThree ? await client.request(FIRST_THREE_ARTICLES) : await client.request(ARTICLES)) || []
  return data
}
